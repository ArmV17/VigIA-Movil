...
@Component({
  ...
  // blog-detail.component.ts (Template parcial)
  template: `
    ...
    <ion-content style="--background: #0a192f; color: white;">
      <img *ngIf="post.encabezado" [src]="post.encabezado" style="width: 100%; height: 250px; object-fit: cover;">
      
      <div style="padding: 20px;">
        <h1 style="color: #3b82f6; font-weight: 800;">{{ post.titulo }}</h1>
        <p style="color: #8a9bb8;">Por: {{ post.autor }} | {{ post.creacion | date }}</p>
        <hr style="border-top: 1px solid rgba(255,255,255,0.1); margin: 20px 0;">
        <div style="line-height: 1.6; color: #cbd5e1;">{{ post.contenido }}</div>
      </div>
    </ion-content>
  `
})
